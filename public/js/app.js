...
for (let oneInput of formElements) {
	...
	// 4d. Validamos el campo de correo electrónico
	if (oneInput.name === 'email') {
		// 4d-I. Asignamos el evento change al campo
		oneInput.addEventListener('blur', function () {
			// 4d-II. Capturo el valor del campo
			let inputEmailValue = this.value;			
			// 4d-III. Validamos si NO está vacío y si es NO un formato de correo electrónico
			if (!validator.isEmpty(inputEmailValue) && !validator.isEmail(inputEmailValue)) {
				// 4d-IV. Agregamos la clase "is-invalid" y eliminamos la clase "is-valid"
				this.classList.add('is-invalid');
				this.classList.remove('is-valid');
				// 4d-V. Al <p> que está inmediatamente después del campo le agregamos el texto de error
